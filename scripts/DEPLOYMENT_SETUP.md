...
The production deployment workflows pass `MSG91_WIDGET_ENABLED=true` and the
active non-secret `MSG91_WIDGET_ID`. `deploy-ecs.sh` persists them as the
`Msg91__WidgetEnabled` and `Msg91__WidgetId` container settings. The widget
token and account Authkey remain in Secrets Manager. Set the enabled flag to
`false` in a controlled rollback deployment to restore the legacy mobile OTP
path.

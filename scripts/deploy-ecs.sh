...
rollback() {
    ...
    if [[ "${service_updated}" == "true" && -n "${previous_task_definition}" && -n "${ECS_SERVICE_ARN}" ]]; then
        echo ""
        echo "Deployment failed."
        echo "Requesting ECS Express rollback to:"
...
# ============================================================
# 3/7 Discover ECS Express service
# ============================================================
...
ECS_SERVICE_ARN="$(
    jq -r '.services[0].serviceArn // empty' <<< "${service_json}"
)"

if [[ -z "${ECS_SERVICE_ARN}" ]]; then
    echo "ERROR: Could not determine ECS service ARN."
    exit 1
fi
...
if [[ -n "${express_status}" && "${express_status}" != "ACTIVE" ]]; then
...
if [[ -z "${previous_task_definition}" ]]; then
    echo "ERROR: Could not determine current ECS Express task definition."
...
# ============================================================
# 4/7 Create new task definition
# ============================================================
...
# ============================================================
# 5/7 Update ECS Express service
# ============================================================
...
# IMPORTANT:
# Initialize this before using it because the script uses
# set -u.
deployment_arn=""
...
while true; do
    ...
    deployment_status="$(
        jq -r \
            '.serviceDeployments[0].status // empty' \
            <<< "${deployment_details}"
    )"

    deployment_reason="$(
        jq -r \
            '.serviceDeployments[0].statusReason // ""' \
            <<< "${deployment_details}"
    )"
